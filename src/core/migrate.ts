...
`);
  ...
  {
    version: 125,
    name: 'take_proposals_empty_status',
    // Zero-result proposal runs persist an `empty` sentinel so unchanged pages
    // do not repeatedly spend an LLM call. Widen the existing status check
    // without rewriting any previously released migration.
    idempotent: true,
    sql: `
