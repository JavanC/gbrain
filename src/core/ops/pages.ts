...
const put_page: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // Remote writes skip inline link/timeline extraction by design; the
    // durable enrichment queue picks the page up out-of-band instead, so the
    // graph still compounds from agent writes.
    let asyncEnrichment:
