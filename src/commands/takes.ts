...
async function applyAcceptPlan(engine: BrainEngine, brainDir: string, previews: TakeProposalAcceptPreview[]): Promise<TakeProposalAcceptPreview[]> {
  // One promotion = one addTakeToPage call: it owns the page lock, the fence
  // round-trip guard, the source-aware write root, and the DB mirror (a failed
  // mirror is a warning healed by the next reconcile, never a duplicate
  // markdown row). The proposal row flips to accepted only after its take is
  // durable on disk — and only `AND status = 'pending'`, so a concurrent
  // reviewer can't double-promote — which leaves an interrupted run with the
  // rest still pending and nothing written twice.
