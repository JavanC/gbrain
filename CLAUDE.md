...
## FORK-LOCAL DEVIATIONS (JavanC/gbrain) — read before acting on any IRON RULE below

This file is upstream's, inherited wholesale on every rebase. Two of its IRON
RULES reference tooling that **does not exist in this fork**. They are upstream
maintainer workflow, not decisions made here. Do not stall on them; do not try
to satisfy them by hand-rolling an approximation of the missing tool.

- **`/ship` does not exist here.** It is a private skill in the upstream
  maintainer's own environment — not in `skills/`, not in `.claude/`, not in
  `~/.claude/skills`, not anywhere in this repo. The instructions to "use
  `/ship`" and "NEVER hand-roll ship operations" arrived via
  `041d89ba` (v0.41.29.0) and `d4211f41` (v0.42.11.0), both authored upstream.
  **In this fork, releases follow `docs/RELEASING.md` manually.** The version
  trio (VERSION / package.json / CHANGELOG) and its 3-line audit still apply —
  those are real, checkable, and CI enforces the first two. What you cannot run
  is `/ship`'s wrapper: its pre-landing review, coverage audit, adversarial
  review, and `/document-release` chaser. Say so explicitly in the PR body when
  a release goes out without them, rather than implying they ran.
- **The Conductor branch=workspace rule does not apply.** This is a plain
  `git clone` + `git worktree` checkout with no Conductor installed. Branch
  names here do not have to match directory names, and renaming a branch to
  satisfy that rule would break the worktrees.

When a rebase reintroduces conflicts around this block, keep the block. It
exists because an agent (and the repo owner) lost time to a rule that could not
be satisfied.
...
(FORK: `/ship` does not exist here — see FORK-LOCAL DEVIATIONS above.)
...
**NEVER hand-roll ship operations.** (FORK: `/ship` does not exist in this
repo — see FORK-LOCAL DEVIATIONS above before acting on this paragraph.)
Do not manually run git commit + push + gh pr
...
- Ship, deploy, push, create PR, "commit and ship", "push and ship" → invoke ship (FORK: absent — follow `docs/RELEASING.md` by hand)
